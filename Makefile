...
build:
	...
	$(GOBUILD) -o $(BUILD_DIR)/$(GOOS)/$(GOARCH)/$(BINARY_NAME) ./cmd/datasaver

build-linux:
	@echo "Building for Linux (amd64)..."
	@mkdir -p $(BUILD_DIR)/linux/amd64
	GOOS=linux GOARCH=amd64 $(GOBUILD) -o $(BUILD_DIR)/linux/amd64/$(BINARY_NAME) ./cmd/datasaver

build-darwin-arm64:
	@echo "Building for macOS (arm64)..."
	@mkdir -p $(BUILD_DIR)/darwin/arm64
	GOOS=darwin GOARCH=arm64 $(GOBUILD) -o $(BUILD_DIR)/darwin/arm64/$(BINARY_NAME) ./cmd/datasaver

build-darwin-amd64:
	@echo "Building for macOS (amd64)..."
	@mkdir -p $(BUILD_DIR)/darwin/amd64
	GOOS=darwin GOARCH=amd64 $(GOBUILD) -o $(BUILD_DIR)/darwin/amd64/$(BINARY_NAME) ./cmd/datasaver

build-linux-arm64:
	@echo "Building for Linux (arm64)..."
	@mkdir -p $(BUILD_DIR)/linux/arm64
	GOOS=linux GOARCH=arm64 $(GOBUILD) -o $(BUILD_DIR)/linux/arm64/$(BINARY_NAME) ./cmd/datasaver

build-linux-armv7:
	@echo "Building for Linux (arm/v7)..."
	@mkdir -p $(BUILD_DIR)/linux/armv7
	GOOS=linux GOARCH=arm GOARM=7 $(GOBUILD) -o $(BUILD_DIR)/linux/armv7/$(BINARY_NAME) ./cmd/datasaver
...
run-dev: build
	...
	@./$(BUILD_DIR)/$(GOOS)/$(GOARCH)/$(BINARY_NAME) daemon --verbose
...
release: update-changelog
	@sh -c '\
	  ...
	  for PLATFORM in "linux/amd64" "linux/arm64" "linux/armv7" "darwin/amd64" "darwin/arm64"; do \
	    BINARY_PATH="release/$${PLATFORM}/datasaver"; \
	    ASSET_NAME="datasaver-$${PLATFORM//\//-}"; \
...
release-assets: update-changelog
	@sh -c '\
		...
		MISSING=0; \
		for BINARY in "release/linux/amd64/datasaver" "release/linux/arm64/datasaver" "release/linux/armv7/datasaver" "release/darwin/amd64/datasaver" "release/darwin/arm64/datasaver"; do \
			if [ ! -f "$$BINARY" ]; then \
		...
		if ! gh release create "$$VERSION" \
			...
			release/linux/amd64/datasaver#datasaver-linux-amd64 \
			release/linux/arm64/datasaver#datasaver-linux-arm64 \
			release/linux/armv7/datasaver#datasaver-linux-armv7 \
			release/darwin/amd64/datasaver#datasaver-darwin-amd64 \
			release/darwin/arm64/datasaver#datasaver-darwin-arm64; then \
...
release-upload-assets: update-changelog
	@sh -c '\
	  ...
	  if ! gh release upload "$$VERSION" \
	    ...
	    release/linux/armv7/datasaver#datasaver-linux-armv7 \
	    release/darwin/amd64/datasaver#datasaver-darwin-amd64 \
	    release/darwin/arm64/datasaver#datasaver-darwin-arm64 \
	    --clobber; then \
